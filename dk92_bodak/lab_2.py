...
class rezistor(komponent): 
    ...
    def __repr__(self):#меняет отображение в системе
        return f"Резистор потрібен для додавання опору в {self.om} Om"
...
class capasitor(komponent):
    def __init__ (self, Far, t):
        self.far=Far
        self.temperature(t)
    def __str__(self):#змінює відображення при викликі
        return f"Конденсатор потрібен для додавання ємності в {self.far} Фарад"
